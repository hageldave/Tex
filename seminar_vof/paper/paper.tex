...
In this section the variational model by \verb|\citeauthor{daspaper}| introduced in \cite{daspaper} will be discussed in detail.

Lets start with the data term which uses a combination of brightness and gradient constancy assumption.
\begin{flalign*}
...
p_{\text{grad}}(\flow) &= ||\;\nabla I_1(\x+\flow(\x))-\nabla I_0(\x)\;||_2^2 &&
\end{flalign*}
In this setup, $\gamma$ is used to weight the terms against each other.
As can be seen $p_{\text{brght}}$ and $p_{\text{grad}}$ are quadratic penalizers, which are sensitive to outliers.
To remove the strong influence of outliers on the minimization, the subquadratic charbonnier penalizer $\charbonnier$ is used to 
robustify the constancy assumptions.
\begin{flalign*}
