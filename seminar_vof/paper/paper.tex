...
S_\Phi &= Q \Lambda Q\T = \mat{\ev_1&\ev_2}\mat{\lam_1&0\\0&\lam_2}\mat{\ev_1\T\\ \ev_2\T}&&
\end{flalign}
...
For the structure tensor the image feature would be brightness $\Phi=I$, but as this is the generalization, $\Phi$ can be something else like .
Using the Eigen value decomposition, the orthonormal eigenvectors $\ev_1$ and $\ev_2$ are obtained which summarize the gradient distribution in the neighborhood defined by the weighting function $\omega$.
If the weighting function was a Dirac delta $\omega = \delta$ the Eigen decomposition of the tensor would yield the normalized gradient as first eigenvector.
\begin{flalign*}
&\int \delta(\tau) \nabla\Phi(\x-\tau)\nabla\Phi(\x-\tau)\T \;d\tau = \nabla\Phi(\x)\nabla\Phi(\x)\T&&
\\
...
\subsection{Second Order Regularizer}
The second order term uses ...
\begin{flalign}
\Rsecond(\flow) = \inf_{\aaux,\baux}[]
\end{flalign}
