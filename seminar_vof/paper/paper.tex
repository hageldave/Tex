...
So what is needed to apply the most suitable regularizer depending on the current sequence, is an adaptive scheme which will be explained in the following.
...
To model this behavour of $c$ a sigmoid function of the benefit $\Delta$ of first over second order regularization is used.
\begin{flalign}
...
\begin{flalign}
&\frac{\partial R}{\partial c} = \int_\Omega S_1(\flow) - S_2(\flow,\aaux,\baux) + \phi_\lambda'(c) \;d\x = 0
&&\nonumber\\
&-|\Omega|*\phi_\lambda'(c) = \int_\Omega S_1(\flow) - S_2(\flow,\aaux,\baux)
&&
\end{flalign}
